...
function generateQuestions(opts) {
  ...
  while (questions.length < count && attempts < maxAttempts) {
    ...
    if (operation === 'multiplication') {
      ...
      b = randInt(Math.max(1, min), Math.max(2, max));
      answer = randInt(1, Math.floor(max / Math.max(1, b)) || 1);
      a = b * answer;
      display = `${a} ÷ ${b}`;
...
function generateWorksheet() {
  ...
  currentConfig = { operation, difficulty, layout, min, max, count, showAnswers, doShuffle, timerOn, timerMins };

  // Generate
  currentQuestions = generateQuestions({ operation, min, max, count, difficulty, shuffle: doShuffle });
...
function buildWorksheetHTML(title, subtitle, questions, layout, showAnswers, totalCount) {
  ...
  if (layout === 'horizontal') {
    ...
    let gridClass = isLongText ? 'ws-questions-longtext' : 'ws-questions-vertical-5';
    if (!isLongText && currentConfig.max > 99999) {
        ...
        html += `
          <div class="ws-q-vertical-math">
            <div class="long-division">
              <span class="divisor">${q.b}</span><span class="bracket">)</span>
              <span class="dividend">${q.a}</span>
            </div>
          </div>
        `;
  ...
  function buildAndDownloadPDF() {
    ...
    let cols = isVertical ? 5 : (isLongText ? 2 : 4);
    if (isVertical && !isLongText && config.max > 99999) {
      cols = 3;
    }
    const colW = contentW / cols;
    const rowH = isVertical ? 35 : (isLongText ? 20 : 15);
